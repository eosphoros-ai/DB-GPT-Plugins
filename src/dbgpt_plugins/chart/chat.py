...
def barChart(datas: List, index: List, columns: List):
    try:
        datas_len = len(datas)
        columns_len = len(columns)
        if datas_len == 0:
...
def barChart_excutor(data: None = {"now": [1, 3, 2, 4], "next": [3, 2, 4, 1]}):
    # 构造 DataFrame 对象
...
if __name__ == "__main__":
    ...
    lineChart(
        [3, 1, 3, 4, 2],
        ["2023-05-09", "2023-05-05", "2023-05-08", "2023-05-10", "2023-05-11"],
        ["dbgpt_user"],
    )
